                         ...
                         uint16_t *const_literal_end) /**< end of the const literal group */
{
  ...
  while ((literal_p = (lexer_literal_t *) parser_list_iterator_next (&literal_iterator)))
  {
    ...
    if (char_p != NULL)
    {
      literal_p->u.value = ecma_find_or_create_literal_string (char_p, literal_p->prop.length);
  ...
  if (context_p->status_flags & PARSER_HAS_LATE_LIT_INIT)
  {
    ...
    while ((literal_p = (lexer_literal_t *) parser_list_iterator_next (&literal_iterator)))
    {
      if ((literal_p->status_flags & LEXER_FLAG_LATE_INIT) && literal_p->prop.index >= register_count)
      {
        ...
        ecma_value_t lit_value = ecma_find_or_create_literal_string (char_p, source_data >> 20);
        literal_pool_p[literal_p->prop.index] = lit_value;
      }
    }
  }
                     ...
                     parser_error_location_t *error_location_p) /**< error location */
{
  ...
  {
    JERRY_DEBUG_MSG ("\n--- %s parsing start ---\n\n", (arg_list_p == NULL) ? "Script" : "Function");
  }
#endif /* JERRY_PARSER_DUMP_BYTE_CODE */

  scanner_scan_all (&context, arg_list_p, arg_list_p + arg_list_size, source_p, source_p + source_size);
...
parser_check_anonymous_function_declaration (parser_context_t *context_p) /**< context */
{
  ...
  const ecma_compiled_code_t *bytecode_p =
    (const ecma_compiled_code_t *) (PARSER_GET_LITERAL (literal_index)->u.bytecode_p);
  bool is_anon =
                          ...
                          uint32_t status_flags) /**< status flags */
{
  ecma_compiled_code_t *bytecode_p =
    (ecma_compiled_code_t *) (PARSER_GET_LITERAL (function_literal_index)->u.bytecode_p);
                                        ...
                                        uint32_t status_flags) /**< status flags */
{
  ecma_value_t *func_name_start_p =
    ecma_compiled_code_resolve_function_name ((const ecma_compiled_code_t *) bytecode_p);
                     ...
                     uint32_t parse_opts) /**< ecma_parse_opts_t option bits */
{
  ...
  if (JERRY_CONTEXT (debugger_flags) & JERRY_DEBUGGER_CONNECTED)
  {
    jerry_debugger_send_string (JERRY_DEBUGGER_SOURCE_CODE, JERRY_DEBUGGER_NO_SUBTYPE, source_p, source_size);
  }
#endif /* JERRY_DEBUGGER */

  ecma_compiled_code_t *bytecode_p =
    parser_parse_source (arg_list_p, arg_list_size, source_p, source_size, resource_name, parse_opts, &parser_error);

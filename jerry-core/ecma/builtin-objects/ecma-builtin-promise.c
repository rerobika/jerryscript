                                   ...
                                   bool *done_p) /**< [out] iteratorRecord[[done]] */
{
  ...
  while (true)
  {
    ...
    /* h. */
    ecma_value_t next_promise = call_call_internal_method (resolve_func_p, ctor, &next_val, 1);
  ...
  while (true)
  {
    ...
    if (ecma_is_value_false (next))
    {
      ...
      if (ecma_promise_remaining_inc_or_dec (remaining, false) == 0)
      {
        ...
        /* 2. */
        ecma_value_t resolve_result = call_call_internal_method (ecma_get_object_from_value (capability_p->resolve),
    ...
    /* i. */
    ecma_value_t next_promise = call_call_internal_method (resolve_func_p, ctor, &next_value, 1);

                                ...
                                ecma_length_t arguments_list_len) /**< number of arguments */
{
  ...
  if (!ecma_is_value_undefined (using_iterator))
  {
    ...
    if (ecma_is_constructor (constructor))
    {
      ...
      array = ecma_op_function_construct (constructor_obj_p, call_this_arg, NULL, 0);
    ...
    while (true)
    {
      ...
      if (mapfn_obj_p != NULL)
      {
        ...
        {
          // TODO: Add iteratorClose support
          goto iterator_cleanup;
        }
      }
      else
...
iterator_cleanup:
  ...
  /* 10. */
  ecma_value_t len_value = ecma_op_object_get_by_magic_id (array_like_obj_p, LIT_MAGIC_STRING_LENGTH);
  ecma_length_t len;

  /* 11. */
  if (ECMA_IS_VALUE_ERROR (len_value)
      || ECMA_IS_VALUE_ERROR (ecma_op_to_length (len_value, &len)))
  {
    ecma_free_value (len_value);
    goto cleanup;
  }
  ...
  if (ecma_is_constructor (constructor))
  {
    ...
    array = ecma_op_function_construct (constructor_obj_p, call_this_arg, &len_value, 1);
...
iterator_cleanup:
  ...
  /* 11. */
  if (ECMA_IS_VALUE_ERROR (len_value)
      || ECMA_IS_VALUE_ERROR (ecma_get_number (len_value, &length_number)))
  {
    ecma_free_value (len_value);
    goto cleanup;
  }

  uint32_t len = ecma_number_to_uint32 (length_number);
  ecma_free_value (len_value);

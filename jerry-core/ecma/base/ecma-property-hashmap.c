...
/**
 * Mark an entry that is deleted.
 */
#define ECMA_PROPERTY_HASHMAP_DIRTY_ENTRY UINT16_MAX

/**
 * Mark an entry that is clean.
 */
#define ECMA_PROPERTY_HASHMAP_CLEAN_ENTRY (UINT16_MAX - 1)
...
ecma_property_hashmap_create (ecma_object_t *object_p) /**< object */
{
  ...
  size_t enrty_size = ECMA_PROPERTY_HASHMAP_GET_ENTRY_SIZE (max_property_count);
  ...
  hashmap_p-type_flags = ECMA_PROPERTY_TYPE_HASHMAP;
  hashmap_p->property_list_cp = prop_iter_cp;
  ...
  uint32_t *pair_list_p = (uint32_t *) (hashmap_p + 1);

  memset (pair_list_p, ECMA_PROPERTY_HASHMAP_CLEAN_ENTRY, entry_size);
  ...
  for (ecma_property_index_t i = 0; i < property_count; i++)
  {
    JERRY_ASSERT (i < ECMA_PROPERTY_HASHMAP_CLEAN_ENTRY);
    ...
    if (curr_property_p->type_flags != ECMA_PROPERTY_TYPE_DELETED)
    {
      ...
      while (pair_list_p[entry_index] < ECMA_PROPERTY_HASHMAP_CLEAN_ENTRY)
      {
...
ecma_property_hashmap_free (ecma_object_t *object_p) /**< object */
{
  ...
  ecma_property_t *property_p = ECMA_GET_NON_NULL_POINTER (ecma_property_t, object_p->u1.property_list_cp);

  JERRY_ASSERT (property_p->type_flags == ECMA_PROPERTY_TYPE_HASHMAP);

  ecma_property_hashmap_t *hashmap_p = (ecma_property_hashmap_t *) property_p;

  object_p->u1.property_list_cp = hashmap_p->property_list_cp;
...
ecma_property_hashmap_insert (ecma_object_t *object_p, /**< object */
                              ...
                              int index) /**< index of the property in the property list */
{
  ...
  JERRY_ASSERT (hashmap_p->type_flags == ECMA_PROPERTY_TYPE_HASHMAP);
                              ...
                              ecma_property_t *property_p) /**< property */
{
  ...
  while (true)
  {
    if (pair_list_p[entry_index] < ECMA_PROPERTY_HASHMAP_CLEAN_ENTRY)
    {
      ...
      ecma_property_t *curr_property_p = property_start_p + property_index;
                            ...
                            jmem_cpointer_t *property_real_name_cp) /**< [out] property real name */
{
  ...
  jmem_cpointer_t prop_iter_cp = hashmap_p->property_list_cp;
  ...
  if (ECMA_IS_DIRECT_STRING (name_p))
  {
    ...
    while (true)
    {
      if (pair_list_p[entry_index] != ECMA_PROPERTY_HASHMAP_CLEAN_ENTRY)
      {
        ...
        if (property_index != ECMA_PROPERTY_HASHMAP_DIRTY_ENTRY)
        {
          ecma_property_t *curr_property_p = property_start_p + property_index;

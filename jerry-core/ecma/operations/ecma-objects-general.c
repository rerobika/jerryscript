                                      ...
                                      ecma_preferred_type_hint_t hint) /**< hint on preferred result type */
{
  ...
  if (!ecma_is_value_undefined (exotic_to_prim))
  {
    ...
    ecma_func_args_t func_args = ecma_op_make_call_args (call_func_p, obj_value, &argument, 1);
                                       ...
                                       ecma_preferred_type_hint_t hint) /**< hint on preferred result type */
{
  ...
  for (uint32_t i = 0; i < 2; i++)
  {
    ...
    if (ecma_op_is_callable (function_value))
    {
      ...
      ecma_func_args_t func_args = ecma_op_make_call_args (func_obj_p, ecma_make_object_value (obj_p), NULL, 0);
      call_completion = ecma_op_function_call (&func_args);
    }

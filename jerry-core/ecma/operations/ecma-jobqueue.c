...
ecma_process_promise_reaction_job (ecma_job_promise_reaction_t *job_p) /**< the job to be operated */
{
  ...
  else
  {
    /* 6. */
    handler_result =
      call_call_internal_method (ecma_get_object_from_value (handler), ECMA_VALUE_UNDEFINED, &(job_p->argument), 1);
  }
  ...
  if (ecma_is_value_false (handler) || ECMA_IS_VALUE_ERROR (handler_result))
  {
    ...
    /* 7. */
    status = call_call_internal_method (ecma_get_object_from_value (capability_p->reject),
                                        ECMA_VALUE_UNDEFINED,
                                        &handler_result,
                                        1);
  }
  ...
  {
    /* 8. */
    status = call_call_internal_method (ecma_get_object_from_value (capability_p->resolve),
                                        ECMA_VALUE_UNDEFINED,
                                        &handler_result,
                                        1);
  }

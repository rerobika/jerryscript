 ...
 */
void
ecma_class_object_delete_builtin_property (ecma_object_t *obj_p, /**< object */
                                           ecma_string_t *property_name_p) /**< property name */
{
  uint8_t type = ((ecma_extended_object_t *) obj_p)->u.cls.type;
  ...
  if (type <= ECMA_OBJECT_CLASS_ARGUMENTS)
  {
    CLASS_OBJ_VTABLE[type].delete_builtin_property (obj_p, property_name_p);
  }
  else
  {
    ecma_ordinary_object_delete_builtin_property (obj_p, property_name_p);
  }
} /* ecma_class_object_delete_property */

                       ...
                       const char *error_msg_p) /**< error message when the function is not callable */
{
  ...
  ecma_value_t iterator = async_generator_object_p->shared.result;
  ecma_value_t result;
...
{
  ecma_object_t *obj_p = ecma_get_object_from_value (async_generator_object_p->shared.result);
...
ecma_async_generator_run (vm_executable_object_t *async_generator_object_p) /**< async generator */
{
  ...
  if (async_generator_object_p->extended_object.u.class_prop.extra_info & ECMA_EXECUTABLE_OBJECT_DO_AWAIT_OR_YIELD)
  {
    switch (task_p->operation_type)
    {
      ...
      {
        result = ecma_op_iterator_next (async_generator_object_p->shared.result,
    ...
    async_generator_object_p->shared.result = ECMA_VALUE_UNDEFINED;
                     ...
                     ecma_value_t value) /**< job value (takes reference) */
{
  ...
  switch (state)
  {
    ...
    {
      ecma_object_t *obj_p = ecma_get_object_from_value (executable_object_p->shared.result);

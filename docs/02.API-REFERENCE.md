...
Enum that contains JerryScript **property filter** options bits:
 ...
 - JERRY_PROPERTY_FILTER_EXLCUDE_STRINGS - Exclude property key if it a string
 - JERRY_PROPERTY_FILTER_EXLCUDE_SYMBOLS - Exclude property key if it a symbol
 - JERRY_PROPERTY_FILTER_EXLCUDE_INTEGER_INDICES - Exclude property key if it an integer index
...
## jerry_object_get_property_names (const jerry_value_t obj_val, jerry_property_filter_t filter);

...
- return
  - reference to the previous realm value - if the passed value is a realm
  - exception - otherwise

*New in version 2.4*.
*Changed in version [[NEXT_RELEASE]]*: A refence is returned for the previous realm instead of the realm itself.
...
Restores the replaced realm with the old one. If the passed value is not a realm reference
nor not the lastly set realm an error is raised.

*Notes*:
- The passed value is always freed.
- This feature depends on build option (`JERRY_BUILTIN_REALMS`) and can be checked
  in runtime with the `JERRY_FEATURE_REALM` feature enum value,
  see: [jerry_is_feature_enabled](#jerry_is_feature_enabled).
- The es.next profile enables this by default.
...
```c
jerry_value_t
jerry_restore_realm (jerry_value_t realm_refernce_value);
```
- `realm_refernce_value` - the old realm reference value
- return
  - undefined - if the passed value is a valid realm reference
  - exception - otherwise

                ...
                const jerry_length_t args_cnt) /**< number of function arguments */
{
  ...
  if (args_cnt > 0 && jerry_value_is_boolean (args_p[0]) && jerry_get_boolean_value (args_p[0]))
  {
...
static void
test_free_cb (void *buffer) /**< buffer to free (if needed) */
{
  (void) buffer;
...
static void
test_detach_free_cb (void *buffer) /**< buffer to free */
{
  free (buffer);
